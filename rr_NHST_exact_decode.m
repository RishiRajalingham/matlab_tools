...
%   'del_fn': all pairwise deltas? 0/1
    ...
    assert( (size(X{1,1},2) == size(Y{1,1},2)) ); 
    
    % Remove nans
    tnan = ~isnan(nanmean(X{1,1})) & ~isnan(nanmean(Y{1,1}));
    nd = sum(tnan);
    ...
    function [X_proj, Y_proj, W] = crossval_linear_remap(X,Y)
        ...
        for iter = 1:niter
            for ki = 1:k
                kj = k-ki+1;
                dat_tr = cat(1, X{iter,ki}(:,tnan), Y{iter,ki}(:,tnan));
                lbl_tr = cat(1, zeros(size(X{iter,ki},1),1), ones(size(Y{iter,ki},1),1));
                dat_te = cat(1, X{iter,kj}(:,tnan), Y{iter,kj}(:,tnan));
